...
begin
  ...
  case
    when upper(p_trans) = 'C' then
      ...
      insert into tcarrera (tcar_cod,tcar_nombre) 
	  values (p_codigo, upper(p_nombre_tipo_carrera));
      commit;
    ...
    when upper(p_trans) = 'A' then
        ...
        UPDATE tcarrera 
        SET tcar_nombre = upper(p_nombre_tipo_carrera) 
		WHERE tcar_cod = p_codigo;
        commit;
  end case;
...
exception 
  ...
  when dup_val_on_index then
    ...
    INSERT INTO AUX_ERROR 
	VALUES (to_char(v_codigo_error),v_mensaje_error,upper(p_trans),'PRO_MANT_TCARRERA',sysdate);
    commit;
...
exception 
  ...
  when ex_fac_presupalto then
    ...
    INSERT INTO AUX_ERROR 
	VALUES (v_codigo_error,v_mensaje_error,upper(p_trans),'PRO_MANT_FACULTAD',sysdate);
    commit;
    raise_application_error(v_codigo_error,v_mensaje_error);
  ...
  when dup_val_on_index then
    ...
    v_mensaje_error := SQLERRM;   
    INSERT INTO AUX_ERROR 
	VALUES (to_char(v_codigo_error), v_mensaje_error,upper(p_trans),'PRO_MANT_FACULTAD',sysdate);
    dbms_output.put_line('error '|| v_codigo_error || ': ' || v_mensaje_error );
    commit;
...
begin
  ...
  case
    ...
    when upper(p_trans) = 'A' then
      UPDATE departamento 
      SET dep_nombre = p_nombre_departamento, dep_fac = p_frac_resp 
	  WHERE dep_cod = p_codigo;
      commit;
  end case;
...
exception 
  ...
  when dup_val_on_index then
    ...
    v_mensaje_error := SQLERRM;
    INSERT INTO AUX_ERROR 
	VALUES(to_char(v_codigo_error),v_mensaje_error, upper(p_trans),'PRO_MANT_DEPARTAMENTO',sysdate);
    dbms_output.put_line('error '|| v_codigo_error || ': ' || v_mensaje_error );
    commit;
...
begin
  ...
  case
    when upper(p_trans) = 'C' then
      ...
      insert into carrera 
	  values (p_codigo,p_nombre_carrera,upper(p_dep_resp),upper(p_tipo_carrera));
      commit;
